...
4. **推理**：综合价格/指标/新闻/基线，输出对"当日交易日"的预测：方向 + 幅度档 + 置信度 + 驱动摘要。归因参考 WGC：动能 24% / 风险 17% / 汇率利率 14% / 经济 12%，央行购金为结构性支撑。
   ...
   **档位校准规则**（2026-08-26 复盘 3 次结算后增设：方向 3/3、档位 1/3，存在系统性偏大）：
   ...
   - **短期超买指标（2026-08-27 增设，复盘 8/26 方向预测错误后引入）**：计算近 20 个交易日累计涨幅，> 8% 标记为"超买区域"（与市况标签 20 日口径一致）。超买区域 + 动能衰减 + 重大事件前夜三重叠加 → 方向从"涨"下调至"跌"的先验提升（而非仅下调档位）；超买区域但无事件催化 → 方向维持但档位额外下调一档。依据：快速上涨积累大量获利盘，一点不及预期即引发获利了结。8/26 教训：20 日涨 14.75% 处超买区域 + 动能连衰 + PCE 前夜，方向本应押跌却押涨，导致方向档位双错。
   - **流动性冲击信号（2026-08-27 增设）**：VIX 单日飙升超 3 点 + 金价同步下跌 → 标记"流动性冲击模式"，档位额外上调一档（黄金比流动性差的资产更易被套现补保证金）。全球主要股指连跌 2 日+ → 标记"流动性冲击预警"，方向偏空先验提升。
   - **市况分层标签（2026-08-27 增设）**：市况标签拆为"长期趋势（60 日方向）+ 短期状态（10 日动能）"，两者背离（如长期涨 + 短期回调）时在报告中明确提示"长期看多但短期调整"，供人工审核参考。
5. **写报告 + 写日志**：写 `predictions/今天.md`（被预测日=生成当天），小节顺序固定、标题一字不差（构建脚本按标题解析）：
   ...
   - `## 🔍 信号分解`（利多/利空逐条 + 幅度档依据；每条信号后标注定价度 `[已定价]` / `[部分定价]` / `[未释放]`，区分已被市场消化的利好与尚未释放的利空）
   - `## ⚠️ 人工审核区`（含场景树：列出 2-3 条条件路径——触发条件 + 概率 + 结果档位，供人工快速定位）
...
- **规则冻结期（2026-09-10 起，至校准样本满 20 个，预计 9/21 前后）**：9/02-9/09 方向连错 5 日，复盘结论="趋势跟随型逻辑在锯齿震荡市失效"（9/02 起市况标签转震荡，20 日涨幅由 +15.76% 收窄至 +1.73%）。依用户决定，冻结期内**任何会话（含定时任务）不得修改预测规则/置信度标定/档位判定**，避免小样本噪声驱动的过度修正（8/26、8/27 两轮规则均有 ≥3 日趋势市数据支撑后才上线）。达标后依 calibration 报告验证并统一实施三条待改项：①震荡市识别降置信度（20 日 ±2% 内 + 近 5 日方向交替 → 置信度 <50%）②震荡市方向不明时默认押小档 ③档位先验重标定（60 日实际"大"档占 54% vs 预测 13 次中仅押 2 次）
