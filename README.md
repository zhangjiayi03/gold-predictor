...
| 置信度(%) | 日志 R 列：晨间生成预测时把报告结论表的方向置信度同步记入；攒够 20 个已结算样本后跑 `scripts/calibration.py` 出校准报告（各档声称把握 vs 实际命中率 + Brier 分数，产出 `reports/calibration_日期.md`），用于检验置信度是否虚标 |
...
│   ├── calibration.py          ← 置信度校准（≥20 已结算样本后出报告，之前只记进度）
...
4. **推理**：综合价格/指标/新闻/基线，输出对"当日交易日"的预测：方向 + 幅度档 + 置信度 + 驱动摘要。归因参考 WGC：动能 24% / 风险 17% / 汇率利率 14% / 经济 12%，央行购金为结构性支撑。
   ...
   **档位校准规则**（2026-08-26 复盘 3 次结算后增设：方向 3/3、档位 1/3，存在系统性偏大）：
   - **先验去趋势化**：60 日"大"占比高反映本轮大涨尾部，判档位时须区分"趋势启动期"（大档先验高）与"高位整理期"（大档先验下调）。近 2 个交易日涨幅递减 → 大档先验下调约 0.5。
   - **动能衰减信号加权**：近 2 日实际涨幅均 < 0.5% 且递减 → "动能衰减"列为档位下调强信号（优先押中/小档）。
   - **事件日历规则**：当日或次晨有 FOMC / PCE / CPI / 非农 / 全球央行年会等重大宏观事件 → 默认档位下调一档（事件前波动收敛），除非已有突发避险拉升。
   - **置信度按档分档标定**：方向置信度趋势市 55%–65%；档位置信度按先验命中难度分档——"小"档 ≤ 35%、"中"档 30%–45%、"大"档 25%–45%。
   - **防响应式过度修正**：档位判断须基于当日信号强度，禁止"上次押大、这次故意押小"的追结果式修正。
